...
class ModalCrearInteres extends StatefulWidget {
  final VoidCallback onSuccess;
  const ModalCrearInteres({super.key, required this.onSuccess});

  @override
  State<ModalCrearInteres> createState() => _ModalCrearInteresState();
}

class _ModalCrearInteresState extends State<ModalCrearInteres> {
  ...
  Future<void> _guardar() async {
    ...
    final creado = await _controller.crearInteres({
      "name": _name.text.trim(),
      "description": _description.text.trim(),
    });

    if (creado) {
      _mostrarMensaje("✅ Interés creado correctamente", verde);
      Future.delayed(const Duration(milliseconds: 400), () {
        Navigator.pop(context);
        widget.onSuccess();
      });
    } else {
      _mostrarMensaje("❌ Error al crear interés", Colors.redAccent);
    }
  }

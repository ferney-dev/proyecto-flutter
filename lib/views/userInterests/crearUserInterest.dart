...
class ModalCrearUserInterest extends StatefulWidget {
  ...
  const ModalCrearUserInterest({super.key, required this.onSuccess});

  @override
  State<ModalCrearUserInterest> createState() => _ModalCrearUserInterestState();
}

class _ModalCrearUserInterestState extends State<ModalCrearUserInterest> {
  ...
  Future<void> _guardar() async {
    ...
    final creado = await _controller.crearUserInterest({
      "userId": _usuarioSeleccionado,
      "interestId": _interesSeleccionado,
    });

    if (creado) {
      _mostrarMensaje("✅ Relación creada correctamente", Colors.green);
      Navigator.pop(context);
      widget.onSuccess();
    } else {
      _mostrarMensaje("❌ Error al crear relación", Colors.redAccent);
    }
  }
